...
class UserServiceImplTest {
  ...
  @Test
  void testFindUserById_whenIdIsNull() {
  ...
  @Test
  void testGetById_whenUserExists_noTickets() {
  ...
  void testUpdateUser_whenUserDoesNotExist() {
    ...
    EntityNotFoundException exception =
        assertThrows(EntityNotFoundException.class, () -> userService.updateUser(1, new User()));
    assertEquals("Client with ID 1 not founded", exception.getMessage());
    verify(userRepository, times(1)).findById(1);
  }
  ...
  void testUpdateUserStatus_whenFlagIsFalse() {
    ...
    IllegalFlagException exception =
        assertThrows(
            IllegalFlagException.class,
            () -> userService.updateUserStatus(1, UserStatus.ACTIVATED));
